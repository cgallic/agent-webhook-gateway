...
class QualityScoreRequest(BaseModel):
    """Request for content quality scoring endpoint."""
    content: str = Field(..., description="Markdown content to score")
    rules: Optional[str] = Field(
        None,
        description="Comma-separated rule sets: aa,geo,structure,four_us"
    )
    model: Optional[str] = Field(
        None,
        description="LLM model for Four U's scoring (default: qwen-plus)"
    )
    use_llm: bool = Field(
        True,
        description="Enable LLM-based rules (Four U's). Set false for fast scoring."
    )
